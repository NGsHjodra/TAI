...
class MyCommunity(Community, PeerObserver):
    ...
    def started(self) -> None:
        ...
        async def send_transaction() -> None:
            receiver = choice(self.get_peers() + [self.my_peer])
            ...
            for peer in self.get_peers():
                if peer != self.my_peer:
                    print(f"[{self.my_peer}] Sending TX to {peer.mid.hex()} amount={amount}")
    ...
    def on_message(self, peer: Peer, payload: Transaction) -> None:
        ...
        print(f"[{self.my_peer}] ✅ Valid TX from {payload.sender_mid.hex()} → {payload.receiver_mid.hex()} amount={payload.amount}")
...
async def start_communities() -> None:
    for i in range(3):  # 3 peers
        ...
        # generate key and write to temp file
        crypto = ECCrypto()
